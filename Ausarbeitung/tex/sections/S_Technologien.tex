...
Um den Implementieraufwand geringer zu halten, wurde entschieden, die Applikation nur für die zwei Betriebssysteme mit dem größten Marktanteil zu implementieren. 
Android und iOS haben gemeinsam einen Anteil von über 50 Prozent. Prototypisch soll die App für Android entwickelt werden inklusive der Möglichkeit iOS in der Zukunft zu unterstützen. Aus dieser 
...
\begin{tabular}{|p{3.5cm}|p{3.5cm} p{3.5cm} p{3.5cm}|} 
	...
	\end{tabular}
\end{center}
\caption{Zusammenfassung der Plattformtechnologien} \label{Zusammenfassung }
\end{table}
...
PhoneGap entstand als Framework zur Entwicklung von Hybriden Apps auf Basis von Web-Technologien wie HTML, CSS und JavaScript. 2011 wurde die Firma Nitobi Software, die PhoneGap einst entwickelte, von Adobe Systems aufgekauft, die das Produkt weiter unter dem Namen Apache Cordova fokussierten. \\
Im Rahmen der hier vorgestellten Arbeit wurde Cordova genutzt, um eine hybride App zu entwickeln. Da der Vorteil des Frameworks vor allem aus seiner Wiederverwendbarkeit besteht. Der geschriebene HTML-, CSS- und JavaScript-Code kann nämlich über Cordova entsprechend der verschiedenen Mobilplattformen unterschiedlich transformiert werden, um die entsprechende Applikation im richtigen Format zu erstellen. Unterstützt werden dabei die verbreitetsten Betriebssysteme Andorid und iOS, aber auch weitere wie Windows Phones, Blackberrys oder sogar FirefoxOS. Damit ist die fertige Applikation später für alle zuvor vorgestellten Betriebssysteme verfügbar.
...
\subsubsection[HTML Geolocation API]{HTML Geolocation API
 \\ \textnormal{\small{\textit {Verfasst von Melanie Hammerschmidt}}}}

Neben den darstellenden Funktionen bietet HTML auch eine Geolocation API, die durch den Prototypen genutzt werden wird. Sie wird auch von allen gängigen Browsern unterstützt bietet aber vor allem auf mobilen Geräten eine sehr hohe Präzision. Die API bietet die Funktionen getCurrentPosition und watchPosition an, welche jeweils die aktuelle Position über GPS bestimmen und durch die Funktion watchPosition auch verfolgen können. Es existiert eine gute Fehlerbehandlung. Wenn z.B. keine Position in einer vorgegebenen Zeit bestimmt werden kann, erfolgt ein Timeout. Die Rückgabedaten beider Funktionen sind:
\begin{itemize}
	\item Längengrad
	\item Breitengrad
	\item Genauigkeit
	\item Höhe
	\item Höhen-Genauigkeit
	\item Orientierung
	\item Geschwindigkeit
	\item Zeitstempel
\end{itemize}
Was die Positionsbestimmung über derartige Schnittstellen so interessant macht ist, dass den jeweiligen Funktionen zusätzlich drei verschiedene Optionsparameter übergeben werden können.
\begin{itemize}
	\item timeout
	\item enableHighAccuracy
	\item maximumAge
\end{itemize}
Im Prototypen werden lediglich die Positionspunkte (Längen- und Breitengrad) genutzt.
\\
Wie bereits erwähnt, bricht ein Lokationsversuch dann ab, wenn die angegebene Timeout-Zeit überschritten wurde. Zudem kann man das maximale Alter einer Positionsinformation beschränken, d.h. wenn aktuell keine Informationen ermittelt werden können werden gecachte Positionsdaten bis zu einem gewissen Alter (maximumAge) zurückgegeben. Über die dritte Option '"enableHighAccuracy'" wird die Fähigkeit freigeschalten, die Positionsbestimmungsmethode zu wählen, die die genauesten Ergebnisse findet. Es wird dann z.B. nicht automatisch über GPS gesucht, was viel Energie verbraucht.
\cite{geoloc}
\\
Die eigentliche Positionsbestimmung funktioniert mithilfe des Cordova-Plugins Geolocation, die die GPS-Funktion des Geräts zugänglich macht. Diesen Zugriff muss der Nutzer natürlich bei Installation zuvor genehmigen.
