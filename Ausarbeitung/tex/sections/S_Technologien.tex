...
Die geographischen Koordinaten geben die Position am genausten an, sind jedoch für fast keine Nutzer einer App von Bedeutung. 
...
Auf Smartphones gehört Kartenmaterial und dessen Integration in Apps mittlerweile zum Standard, an den sich Nutzer gewöhnt haben. Und über die Hälfte der Deutschen, 44 Millionen Menschen, nutzen schon ein Smartphone. Die Tendenz ist dabei steigend. Dies ist das Ergebnis einer Bitkom Studie. \cite{bitkom} Aus diesem Grund sollte auch Kartenmaterial in die Location based Services App integriert werden, welche die Autoren bei dieser Studienarbeit entwickeln. 
...
Die Anforderungen an interaktives Kartenmaterial, bezüglich des in dieser Studienarbeit entwickelten Projektes, lassen sich in zwei Gruppen einteilen: die funktionalen und nichtfunktionalen Anforderungen.
...
\item Eigenen Standort anzeigen\\
Der eigene Standort muss grafisch auf einer interaktiven Karte angezeigt werden. Das Zentrieren des Kartenmaterials auf den eigenen Standort sollte auch möglich sein.
...
\item Ohne Account nutzbar\\
Seit dem die Google Maps JavaScript API in der Version 3 vorliegt ist die Nutzung ohne einen Schlüssel möglich. Das bedeutet, dass keine Registrierung bzw. Anmeldung zum Gebrauch nötig ist. Diese Praxis erleichtert den Einstieg in Google Maps, da man sofort starten kann. \cite{googlemapsblog}
...
Zum ausführlichen Entwicklerhandbuch gibt es nochmals fast 150 Beispiele, zu denen die jeweils passende Output (Karte) angezeigt wird.\cite[Documentation]{googlemaps} \\
Neben der Dokumentation von Google selbst gibt es auch andere Tutorials im Netz. Eines davon ist von der Webseite www.w3schools.com, das versucht von Grund auf eine Einführung in Google Maps zu geben. 
...
Mit Hilfe dieser Bibliothek kann ein \glqq Markercluster \grqq erstellt werden, dass Markierungen bei einer gewissen Zoomstufe bündelt. Hierbei wird dem Markercluster ein Array der Markierungen, eine Refernz auf die Karte, sowie Markercluster-Einstellungen hinzugefügt.
...
\item Markierungen auf der Karte setzen\\
Mit Bing Maps ist es möglich Markierungen auf der Karte zu platzieren. Dieser werden bei hierbei PushPins genannt. Diese Pushpins werden dann zum Anzeigen dem Map Objekt übergeben (Listing Zeile 3). Eine nachträgliche Positionsänderung der gesetzten Pushpins ist möglich, indem man diesem eine neue Position zuweist (Listing Zeile 4).
\begin{lstlisting}
map.entities.clear(); 
